...
import com.feylabs.core.domain.DrinkIngredientCategory.DrinkIngredientCategoryItem as AdapterModel

class IngredientAdapter : RecyclerView.Adapter<IngredientAdapter.AdapterViewHolder>() {
    ...
    inner class AdapterViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        fun onBInd(model: AdapterModel) {
            ...
            binding.tvName.text = model.strIngredient1
            binding.photo.loadImageFromURL(
                mContext,
                mContext.getString(R.string.ingredients_url) + "${model.strIngredient1}.png"
            )
        }
    }
