...
import com.feylabs.core.domain.DrinkGlassCategory.DrinkGlassCategoryItem as AdapterModel
...
class GlassAdapter : RecyclerView.Adapter<GlassAdapter.AdapterViewHolder>() {
    ...
    inner class AdapterViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        fun onBInd(model: AdapterModel) {
            ...
            binding.tvName.text = model.strGlass
            binding.photo.loadImageFromURL(
                mContext,
                mContext.getString(R.string.ingredients_url) + "${model.strGlass}.png"
            )
        }
    }
